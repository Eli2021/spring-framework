...
/**
 * Abstract {@link BeanDefinitionParser} implementation providing a number of convenience methods and a {@link
 * AbstractBeanDefinitionParser#parseInternal template method} that subclasses must override to provide the actual
 * parsing logic.
 *
 * <p>Use this {@link BeanDefinitionParser} implementation when you want
 * to parse some arbitrarily complex XML into one or more {@link BeanDefinition BeanDefinitions}. If you just want to
 * parse some XML into a single {@code BeanDefinition}, you may wish to consider the simpler convenience extensions of
 * this class, namely {@link AbstractSingleBeanDefinitionParser} and {@link AbstractSimpleBeanDefinitionParser}.
 *
 * @author Rob Harrop
...
public abstract class AbstractBeanDefinitionParser implements BeanDefinitionParser {
	 ...
	 * Otherwise, the ID is extracted from the "id" attribute, potentially with a {@link #shouldGenerateIdAsFallback()
	...
	/**
	 * Register the supplied {@link BeanDefinitionHolder bean} with the supplied {@link BeanDefinitionRegistry
	 * registry}.
	 ...
	 * with the supplied {@link BeanDefinitionRegistry registry} only if the {@code isNested} parameter is {@code
	 * false}, because one typically does not want inner beans to be registered as top level beans.
	 *
	 * @param definition the bean definition to be registered
	...
	/**
	 * Central template method to actually parse the supplied {@link Element} into one or more {@link BeanDefinition
	 * BeanDefinitions}.
	 *
	 ...
	 * Note that this flag is about <i>always</i> generating an ID; the parser won't even check for an "id" attribute in
	 * this case.
	 *
	...
	/**
	 * Should an ID be generated instead if the passed in {@link Element} does not specify an "id" attribute
	 * explicitly?
	 * <p>Disabled by default; subclasses can override this to enable ID generation
	 * as fallback: The parser will first check for an "id" attribute in this case, only falling back to a generated ID
	...
	/**
	 * Determine whether the element's "name" attribute should get parsed as bean definition aliases, i.e. alternative
	...
	/**
	 * Determine whether this parser is supposed to fire a {@link org.springframework.beans.factory.parsing.BeanComponentDefinition}
	 * event after parsing the bean definition.
	 * <p>This implementation returns {@code true} by default; that is,
	 * an event will be fired when a bean definition has been completely parsed. Override this to return {@code false}
	 * in order to suppress the event.
	 *
	 * @return {@code true} in order to fire a component registration event after parsing the bean definition; {@code
	...
	/**
	 * Hook method called after the primary parsing of a {@link BeanComponentDefinition} but before the {@link
	 * BeanComponentDefinition} has been registered with a {@link org.springframework.beans.factory.support.BeanDefinitionRegistry}.
